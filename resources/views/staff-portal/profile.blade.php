                ...
                <form id="staff-profile-update-form" method="POST" action="{{ route('staff-portal.profile-update') }}" enctype="multipart/form-data" class="mt-6 grid gap-6">
                    ...
                    <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_18px_45px_rgba(15,23,42,0.06)]">
                        ...
                        <div class="mt-6 grid gap-4">
                            ...
                            <label class="grid gap-2 text-sm font-bold text-slate-800">Previous Experience<textarea class="{{ $inputClass }} {{ $errorClass('experience') }} min-h-32" name="experience" placeholder="Example: Three years delivering commercial cleaning across Melbourne.&#10;Reference: Jane Smith, Facilities Manager, 0400 000 000.&#10;Service area: Melbourne CBD and surrounding suburbs.">{{ $formValue('experience', $staff->experience) }}</textarea>@error('experience')<span class="text-xs font-semibold text-red-600">{{ $message }}</span>@enderror</label>
