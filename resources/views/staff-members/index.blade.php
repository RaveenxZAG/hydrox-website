                        ...
                        <tr>
                            ...
                            <td class="py-3 pr-4">
                                @if ($staff->invoicing_enabled)
                                    <span class="badge bg-emerald-50 text-emerald-800">Enabled</span>
                                @else
                                    <span class="badge bg-slate-100 text-slate-700">Disabled</span>
                                @endif
                            </td>
                            <td class="py-3 pr-4">
                                @if ($staff->missingInfoCount() === 0)
                                    <span class="badge bg-emerald-50 text-emerald-800">Complete</span>
                                @else
                                    <span class="badge bg-rose-50 text-rose-800">{{ $staff->missingInfoCount() }} missing</span>
                                @endif
                            </td>
                            <td class="py-3 pr-4">
                                <span class="badge {{ $staff->staff_status === 'active' ? 'bg-emerald-50 text-emerald-800' : 'bg-slate-100 text-slate-700' }}">{{ ucfirst($staff->staff_status ?: 'active') }}</span>
                            </td>
                            <td class="py-3 text-right">
                                <a class="font-semibold text-[#006da9]" href="{{ route('staff-members.show', $staff) }}">View</a>
                            </td>
                        </tr>
                    @empty
