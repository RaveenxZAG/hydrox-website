...
<div x-data='reportBuilder(@json($existing))' data-draft-key="{{ $existing['draft_key'] }}" @report-submit-start.document="startSubmit" class="grid gap-6">
    <div x-show="submitting" x-cloak class="fixed inset-0 z-50 grid place-items-center bg-slate-950/70 px-4 backdrop-blur-sm">
        <div class="w-full max-w-md rounded-lg border border-slate-200 bg-white p-6 shadow-2xl">
            <div class="mb-4 flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-lg font-bold text-slate-950">Saving report</h2>
    ...
    <x-card>
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <x-field class="md:col-span-2 xl:col-span-3" label="Job Number" name="job_id">
                <div class="relative" @click.outside="jobResultsOpen = false">
                    ...
                    <input
                        ...
                        autocomplete="off"
                    >
                    <div x-show="jobResultsOpen" x-cloak class="absolute z-30 mt-2 max-h-80 w-full overflow-y-auto rounded-xl border border-slate-200 bg-white p-2 shadow-2xl">
                        <p x-show="jobSearching" class="px-3 py-4 text-sm text-slate-500">Searching jobs...</p>
                        <template x-for="job in jobResults" :key="job.id">
                            <button type="button" class="block w-full rounded-lg px-3 py-3 text-left transition hover:bg-[#eaf6fc]" @click="selectJob(job)">
                                <span class="block font-bold text-slate-950" x-text="job.job_number"></span>
                                <span class="mt-1 block text-sm text-slate-600" x-text="[job.customer?.name || job.customer?.company || 'Missing client', job.cleaning_service || 'No service'].join(' · ')"></span>
            ...
            <div x-show="selectedJob" x-cloak class="md:col-span-2 xl:col-span-3 grid gap-4 rounded-xl border border-[#b8dff3] bg-[#eaf6fc]/70 p-4 lg:grid-cols-2">
                <div>
                    <p class="text-xs font-bold uppercase tracking-wide text-[#006da9]">Client Details</p>
                    <p class="mt-2 font-bold text-slate-950" x-text="selectedJob?.customer?.name || selectedJob?.customer?.company || 'Not recorded'"></p>
                    <p class="mt-1 text-sm text-slate-600" x-text="selectedJob?.customer?.company || ''"></p>
                    <p class="mt-1 text-sm text-slate-600" x-text="selectedJob?.customer?.phone || 'No phone recorded'"></p>
                    <p class="text-sm text-slate-600" x-text="selectedJob?.customer?.email || 'No email recorded'"></p>
                    <p class="text-sm text-slate-600" x-text="selectedJob?.customer?.address || 'No address recorded'"></p>
                </div>
                <div>
                    <p class="text-xs font-bold uppercase tracking-wide text-[#006da9]">Job Details</p>
                    <p class="mt-2 font-bold text-slate-950" x-text="selectedJob?.cleaning_service || 'No service recorded'"></p>
                    <p class="mt-1 text-sm text-slate-600" x-text="['Job ' + (selectedJob?.job_number || ''), selectedJob?.booking_date].filter(Boolean).join(' · ')"></p>
                    <p class="text-sm text-slate-600" x-text="[selectedJob?.start_time, selectedJob?.finish_time].filter(Boolean).join(' – ') || 'No time recorded'"></p>
                    <p class="text-sm text-slate-600" x-text="selectedJob?.technician ? 'Technician: ' + selectedJob.technician : 'No technician recorded'"></p>
                    <p class="text-sm text-slate-600" x-text="selectedJob?.status ? 'Status: ' + selectedJob.status : ''"></p>
                </div>
            </div>
            ...
            <label class="flex items-center gap-3 pt-7 text-sm font-medium"><input type="hidden" name="customer_present" value="0"><input class="rounded border-slate-300 text-[#0082c9] focus:ring-[#0082c9]" type="checkbox" name="customer_present" value="1" @checked(old('customer_present', $report->customer_present))> Client present</label>
        </div>
    </x-card>
    ...
    <x-card>
        ...
        <div class="grid gap-4">
            <div class="relative">
                ...
                <div class="mt-2 grid gap-2 rounded-lg border border-slate-200 bg-white p-2" x-show="checklistSuggestions().length">
                    <template x-for="item in checklistSuggestions()" :key="item">
                        <button type="button" class="rounded-md px-3 py-2 text-left text-sm font-medium text-slate-700 hover:bg-[#eaf6fc] hover:text-[#07527d]" @click="addChecklistItem(item)" x-text="item"></button>
                    </template>
                </div>
            </div>
            ...
            <div class="flex flex-wrap gap-2" x-show="checklist.length">
                <template x-for="(item, index) in checklist" :key="`${item}-${index}`">
                    <span class="inline-flex items-center gap-2 rounded-full border border-[#b8dff3] bg-[#eaf6fc] px-3 py-1.5 text-sm font-semibold text-[#07527d]">
                        <input type="hidden" name="checklist[]" :value="item">
                        <span x-text="item"></span>
                        <button type="button" class="text-[#0b2a4a]/70 hover:text-rose-600" @click="removeChecklistItem(index)" :aria-label="`Remove ${item}`">x</button>
                    </span>
                </template>
            </div>
            ...
            <div class="mb-5 rounded-lg border border-slate-200 p-4">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    ...
                    <div class="grid gap-2">
                        ...
                        <label class="grid cursor-pointer place-items-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 px-4 py-6 text-center transition hover:border-cyan-400 hover:bg-[#eaf6fc]"
                            @dragover.prevent="$event.currentTarget.classList.add('border-[#0082c9]', 'bg-[#eaf6fc]', '')"
                            @dragleave.prevent="$event.currentTarget.classList.remove('border-[#0082c9]', 'bg-[#eaf6fc]', '')"
                            @drop.prevent="$event.currentTarget.classList.remove('border-[#0082c9]', 'bg-[#eaf6fc]', ''); dropFiles($event, `areas[${index}][before_photos][]`, $event.currentTarget.querySelector('input[type=file]'), $event.currentTarget.nextElementSibling)">
                            <span class="text-sm font-semibold text-slate-800">Drop before photos here</span>
                        ...
                        <div class="flex flex-wrap gap-2" x-show="area.before_photos?.length">
                            <template x-for="photo in area.before_photos" :key="photo.id">
                                <div class="relative h-24 w-24 overflow-hidden rounded-lg ring-1 ring-slate-200">
                    ...
                    <div class="grid gap-2">
                        ...
                        <label class="grid cursor-pointer place-items-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 px-4 py-6 text-center transition hover:border-cyan-400 hover:bg-[#eaf6fc]"
                            @dragover.prevent="$event.currentTarget.classList.add('border-[#0082c9]', 'bg-[#eaf6fc]', '')"
                            @dragleave.prevent="$event.currentTarget.classList.remove('border-[#0082c9]', 'bg-[#eaf6fc]', '')"
                            @drop.prevent="$event.currentTarget.classList.remove('border-[#0082c9]', 'bg-[#eaf6fc]', ''); dropFiles($event, `areas[${index}][after_photos][]`, $event.currentTarget.querySelector('input[type=file]'), $event.currentTarget.nextElementSibling)">
                            <span class="text-sm font-semibold text-slate-800">Drop after photos here</span>
                            <span class="mt-1 text-xs text-slate-500">or click to choose photos</span>
    ...
    <x-card>
        ...
        <template x-for="(issue, index) in issues" :key="index">
            <div class="mb-3 grid gap-3 rounded-lg border border-slate-200 p-3 lg:grid-cols-5">
                ...
                <div class="grid gap-2">
                    <label class="grid cursor-pointer place-items-center rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 px-3 py-4 text-center transition hover:border-cyan-400 hover:bg-[#eaf6fc]"
                        @dragover.prevent="$event.currentTarget.classList.add('border-[#0082c9]', 'bg-[#eaf6fc]', '')"
                        @dragleave.prevent="$event.currentTarget.classList.remove('border-[#0082c9]', 'bg-[#eaf6fc]', '')"
                        @drop.prevent="$event.currentTarget.classList.remove('border-[#0082c9]', 'bg-[#eaf6fc]', ''); dropFiles($event, `issues[${index}][photos][]`, $event.currentTarget.querySelector('input[type=file]'), $event.currentTarget.nextElementSibling)">
                        <span class="text-xs font-semibold text-slate-800">Drop issue photos</span>
        ...
        <x-card>
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                ...
                <div x-data="signatureBox('technician_signature')" class="grid gap-2 lg:col-span-2">
                    ...
                    <canvas x-ref="canvas" class="h-40 w-full rounded-lg border border-slate-300 bg-white"></canvas>
    ...
    <div class="sticky bottom-0 -mx-4 border-t border-slate-200 bg-white/95 px-4 py-4 backdrop-blur sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
