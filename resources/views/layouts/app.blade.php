...
<body class="hydrox-theme">
    ...
    <div x-data="{ sidebarOpen: false }" class="min-h-screen bg-[#eef3f6] text-[#10243a]">
        ...
        <aside
            class="fixed inset-y-0 left-0 z-40 flex w-72 -translate-x-full flex-col overflow-hidden border-r border-slate-200 bg-white text-slate-700 shadow-2xl shadow-slate-900/10 transition-transform duration-300 ease-out lg:translate-x-0"
            :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
        >
            <div class="hydrox-brand-rail h-1"></div>

            <div class="relative border-b border-slate-200 px-3 py-3">
                <div class="flex items-center justify-between gap-3">
                    <a href="{{ route('dashboard') }}" class="flex min-w-0 flex-1 items-center gap-3">
                        <span class="grid h-12 w-36 shrink-0 place-items-center rounded-xl border border-slate-200 bg-white px-2.5 py-1.5 shadow-sm">
                            <img class="h-full w-full rounded-lg object-contain" src="{{ asset('images/hydrox-logo.svg') }}" alt="Hydrox logo">
                        </span>
                    </a>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button class="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-600 transition hover:border-rose-200 hover:bg-rose-50 hover:text-rose-700">
                            Logout
                        </button>
                    </form>
                </div>

                <label class="mt-3 flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-500">
                    ...
                    <input class="w-full border-0 bg-transparent p-0 text-sm text-slate-800 placeholder:text-slate-400 focus:ring-0" type="search" placeholder="Search">
                </label>

                <div class="mt-3 rounded-lg border border-[#0082c9]/15 bg-[#0082c9]/5 px-3 py-2">
                    <p class="text-xs font-bold uppercase tracking-wide text-[#0082c9]">Workspace</p>
                    <p class="mt-0.5 text-xs leading-5 text-slate-500">Bookings and business operations.</p>
                </div>
            </div>
            ...
            <nav class="relative min-h-0 flex-1 overflow-y-auto px-2.5 py-3">
                ...
                <div class="grid gap-1">
                    @foreach ($navItems as $item)
                        ...
                        <a href="{{ route($item['route']) }}" class="group relative flex h-10 items-center gap-3 rounded-xl px-3 text-sm font-semibold transition {{ $active ? 'bg-[#0082c9]/10 text-slate-950 shadow-sm ring-1 ring-[#0082c9]/15' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950' }}">
                            @if ($active)
                                <span class="absolute left-0 top-2 bottom-2 w-1 rounded-r-full bg-[#0082c9]"></span>
                            @endif
                            <span class="grid h-7 w-7 shrink-0 place-items-center rounded-lg {{ $active ? 'bg-white text-[#0082c9] shadow-sm' : 'bg-slate-100 text-slate-500 group-hover:text-[#0082c9]' }}">
                ...
                <div x-data="{ open: {{ request()->routeIs('emails.*') ? 'true' : 'false' }} }" class="mt-1">
                    ...
                    <button type="button" class="group relative flex h-10 w-full items-center gap-3 rounded-xl px-3 text-left text-sm font-semibold transition {{ $emailActive ? 'bg-[#0082c9]/10 text-slate-950 shadow-sm ring-1 ring-[#0082c9]/15' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950' }}" @click="open = !open" :aria-expanded="open">
                        @if ($emailActive)
                            <span class="absolute left-0 top-2 bottom-2 w-1 rounded-r-full bg-[#0082c9]"></span>
                        @endif
                        <span class="grid h-7 w-7 shrink-0 place-items-center rounded-lg {{ $emailActive ? 'bg-white text-[#0082c9] shadow-sm' : 'bg-slate-100 text-slate-500 group-hover:text-[#0082c9]' }}">
                            {!! $sidebarIcon('email') !!}
                        </span>
                    ...
                    <div x-show="open" x-cloak class="ml-[26px] mt-1 grid gap-0.5 border-l border-slate-200 pl-4">
                        <a href="{{ route('emails.create') }}" class="relative flex h-8 items-center rounded-lg px-3 text-[13px] font-semibold transition {{ $emailActive ? 'bg-[#0082c9]/8 text-[#0082c9]' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-950' }}">
                            @if ($emailActive)
                ...
                <div x-data="{ open: {{ request()->routeIs('subcontractor-onboardings.*') || request()->routeIs('staff-members.*') || request()->routeIs('staff-profile-changes.*') ? 'true' : 'false' }} }" class="mt-1">
                    ...
                    <button type="button" class="group relative flex h-10 w-full items-center gap-3 rounded-xl px-3 text-left text-sm font-semibold transition {{ $staffActive ? 'bg-[#0082c9]/10 text-slate-950 shadow-sm ring-1 ring-[#0082c9]/15' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950' }}" @click="open = !open" :aria-expanded="open">
                        @if ($staffActive)
                            <span class="absolute left-0 top-2 bottom-2 w-1 rounded-r-full bg-[#0082c9]"></span>
                        @endif
                        <span class="grid h-7 w-7 shrink-0 place-items-center rounded-lg {{ $staffActive ? 'bg-white text-[#0082c9] shadow-sm' : 'bg-slate-100 text-slate-500 group-hover:text-[#0082c9]' }}">
                            {!! $sidebarIcon('staff') !!}
                        </span>
                ...
                <div x-data="{ open: {{ request()->routeIs('staff-invoices.*') ? 'true' : 'false' }} }" class="mt-1">
                    @php
                        ...
                        $invoiceLinks = [
                            ...
                            [
                                ...
                                'badge' => 'bg-[#0082c9]/10 text-[#0082c9] ring-1 ring-[#0082c9]/20',
                            ],
                            [
                    ...
                    @endphp
                    <button type="button" class="group relative flex h-10 w-full items-center gap-3 rounded-xl px-3 text-left text-sm font-semibold transition {{ $invoiceActive ? 'bg-[#0082c9]/10 text-slate-950 shadow-sm ring-1 ring-[#0082c9]/15' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950' }}" @click="open = !open" :aria-expanded="open">
                        @if ($invoiceActive)
                            <span class="absolute left-0 top-2 bottom-2 w-1 rounded-r-full bg-[#0082c9]"></span>
                        @endif
                        <span class="grid h-7 w-7 shrink-0 place-items-center rounded-lg {{ $invoiceActive ? 'bg-white text-[#0082c9] shadow-sm' : 'bg-slate-100 text-slate-500 group-hover:text-[#0082c9]' }}">
                    ...
                    <div x-show="open" x-cloak class="ml-[26px] mt-1 grid gap-0.5 border-l border-slate-200 pl-4">
                        @foreach ($invoiceLinks as $item)
                            <a href="{{ $item['href'] }}" class="relative flex h-8 items-center justify-between gap-2 rounded-lg px-3 text-[13px] font-semibold transition {{ $item['active'] ? 'bg-[#0082c9]/8 text-[#0082c9]' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-950' }}">
                                @if ($item['active'])
                                    <span class="absolute -left-[17px] top-1/2 h-1.5 w-1.5 -translate-y-1/2 rounded-full bg-[#0082c9]"></span>
                                @endif
                                <span class="truncate">{{ $item['label'] }}</span>
                                @if (! is_null($item['count']) && $item['count'] > 0)
                                    <span class="grid h-5 min-w-5 shrink-0 place-items-center rounded-full bg-slate-100 px-1.5 text-[11px] font-bold text-slate-600 ring-1 ring-slate-200">{{ $item['count'] > 99 ? '99+' : $item['count'] }}</span>
                                @endif
                            </a>
                        @endforeach
                    </div>
                ...
                <div x-data="{ open: {{ request()->routeIs('invoice-template.*') || request()->routeIs('settings.*') ? 'true' : 'false' }} }" class="mt-1">
                    ...
                    <button type="button" class="group relative flex h-10 w-full items-center gap-3 rounded-xl px-3 text-left text-sm font-semibold transition {{ $settingsActive ? 'bg-[#0082c9]/10 text-slate-950 shadow-sm ring-1 ring-[#0082c9]/15' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-950' }}" @click="open = !open" :aria-expanded="open">
                        @if ($settingsActive)
                            <span class="absolute left-0 top-2 bottom-2 w-1 rounded-r-full bg-[#0082c9]"></span>
                        @endif
                        <span class="grid h-7 w-7 shrink-0 place-items-center rounded-lg {{ $settingsActive ? 'bg-white text-[#0082c9] shadow-sm' : 'bg-slate-100 text-slate-500 group-hover:text-[#0082c9]' }}">
            ...
            <div x-data="{ portalsOpen: false }" class="relative border-t border-slate-200 px-3 pt-3">
                <button
                    type="button"
                    class="group flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-semibold text-slate-600 transition hover:bg-slate-50 hover:text-slate-950"
                    @click="portalsOpen = ! portalsOpen"
                    :aria-expanded="portalsOpen"
                    aria-controls="company-portals-panel"
                >
                    <span class="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-[#eaf6fc] text-[#0082c9]">
                        {!! $sidebarIcon('portals') !!}
                    </span>
                ...
                <div id="company-portals-panel" x-show="portalsOpen" x-cloak class="mt-2 grid gap-1.5 rounded-xl border border-slate-200 bg-slate-50 p-2">
                    @foreach ($companyPortals as $portal)
                        <a
                            class="group flex items-start gap-3 rounded-lg bg-white px-3 py-2.5 shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:ring-[#79c5e9]"
                            href="{{ $portal['url'] }}"
                            target="_blank"
                            rel="noopener noreferrer"
                        >
                            <span class="mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-lg {{ $portal['current'] ? 'bg-emerald-50 text-emerald-700' : 'bg-[#eaf6fc] text-[#0082c9]' }}">
                                @switch($portal['icon'])
                                    @case('admin')
                            ...
                            <span class="min-w-0 flex-1">
                                <span class="flex items-center gap-2">
                                    <span class="truncate text-xs font-black text-slate-900">{{ $portal['label'] }}</span>
                                    @if ($portal['current'])
                                        <span class="rounded-full bg-emerald-50 px-1.5 py-0.5 text-[9px] font-black uppercase tracking-wide text-emerald-700">Current</span>
                                    @endif
                                </span>
                                <span class="mt-0.5 block text-[10px] leading-4 text-slate-500">{{ $portal['description'] }}</span>
                                <span class="mt-1 block truncate text-[10px] font-semibold text-[#0082c9]">{{ $portal['host'] }}</span>
                            </span>
            ...
            <div class="relative p-3"><div class="rounded-xl border border-[#0082c9]/15 bg-[#eaf6fc] px-3 py-2 text-xs font-semibold text-[#0b2a4a]">Hydrox Facility Management</div></div>
        </aside>
            ...
            <header class="sticky top-0 z-20 border-b border-slate-200/80 bg-white/90 backdrop-blur">
                <div class="flex min-h-16 flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
                    <div class="flex min-w-0 items-center gap-3">
                            <button type="button" class="grid h-10 w-10 place-items-center rounded-lg border border-slate-200 bg-white text-xs font-black uppercase text-slate-700 shadow-sm lg:hidden" @click="sidebarOpen = true" aria-label="Open menu">
                            Menu
                        </button>
                        <div class="min-w-0">
                            <p class="text-xs font-semibold uppercase tracking-wide text-slate-400">@yield('eyebrow', 'Hydrox Portal')</p>
                            <h1 class="truncate text-xl font-black tracking-tight sm:text-2xl">@yield('title', 'Dashboard')</h1>
                        </div>
                    </div>
                    <div class="flex flex-wrap items-center gap-2">
                        <div x-data="{ open: false }" class="relative">
                            <button type="button" class="relative grid h-10 w-10 place-items-center rounded-lg border shadow-sm transition {{ $systemNotificationCount > 0 ? 'border-rose-200 bg-rose-50 text-rose-700 hover:border-rose-300 hover:bg-rose-100' : 'border-slate-200 bg-white text-slate-600 hover:border-[#0082c9]/30 hover:bg-[#0082c9]/10 hover:text-slate-950' }}" @click="open = !open" @click.outside="open = false" aria-label="Notifications">
                                ...
                                @if ($systemNotificationCount > 0)
                                    <span class="absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-rose-600 px-1 text-[10px] font-bold text-white ring-2 ring-white">
                                        {{ $systemNotificationCount > 9 ? '9+' : $systemNotificationCount }}
                                    </span>
                                @endif
                            </button>

                            <div x-show="open" x-cloak class="absolute right-0 z-30 mt-3 w-80 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/15">
                                ...
                                <div class="max-h-96 overflow-y-auto p-2">
                                    @forelse ($systemNotifications as $notification)
                                        <a href="{{ route('notifications.open', $notification) }}" class="block rounded-lg px-3 py-3 transition hover:bg-slate-50">
                ...
                <div class="fixed left-1/2 top-20 z-50 grid w-[calc(100%-2rem)] max-w-xl -translate-x-1/2 gap-3">
                    @foreach ($flashMessages as $flash)
                        <div
                            ...
                            x-show="show"
                            x-transition
                            class="rounded-xl border px-4 py-3 text-sm shadow-2xl shadow-slate-900/15 backdrop-blur {{ $flash['type'] === 'success' ? 'border-emerald-200 bg-emerald-50/95 text-emerald-800' : ($flash['type'] === 'warning' ? 'border-amber-200 bg-amber-50/95 text-amber-800' : 'border-rose-200 bg-rose-50/95 text-rose-800') }}"
                        >
                            <div class="flex items-start justify-between gap-3">
